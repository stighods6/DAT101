...
export class TTile extends libSprite.TSpriteButton {
  ...
  onMouseUp(aEvent) {
    if (aEvent.button === 2) {
      if (this.index === 3){
        this.index = 0;
        gameProps.ScoreBoard.MineInfoColors++;
    } else {
  ...
  if(this.#isMine) {
    ...
    forEachTile( this.#countOpenTile);
    const totalTiles = gameProps.tiles.length;
    const tileLeft = totalTiles - gameProps.openTiles;
    if(tileLeft === gameLevel.Mines){
      gameProps.ScoreBoard.spSmiley.index = 3;
      setGameOver();
    }
    }
